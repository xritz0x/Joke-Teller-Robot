...
const playVoice = () => {
    VoiceRSS.speech({
        key: '5be32ae69f6e444ba7fb2e282cefdcab',
        src: srcVar,
        hl: 'en-us',
        v: 'Linda',
        r: 0, 
...
// toggle button.disabled value
const toggleButton = () => {
    button.disabled = !button.disabled;
}
const buttonFunctionality = async () => {
    toggleButton();
    try {
        const ftch = await fetch('https://v2.jokeapi.dev/joke/Programming,Dark,Christmas?type=single');
        const repsonse = await ftch.json();
        srcVar = repsonse.joke;
        playVoice();        
    } catch (error) {
...
button.addEventListener('click', buttonFunctionality);
